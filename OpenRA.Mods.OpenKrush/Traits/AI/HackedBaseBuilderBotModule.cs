...
namespace OpenRA.Mods.OpenKrush.Traits.AI
{
		...
		IBotTick, IBotPositionsUpdated, IBotRespondToAttack, IBotRequestPauseUnitProduction
	{
		...
		List<HackedBaseBuilderQueueManager> builders = new List<HackedBaseBuilderQueueManager>();

		public HackedBaseBuilderBotModule(Actor self, HackedBaseBuilderBotModuleInfo info)
			: base(info)
		{
			world = self.World;
			player = self.Owner;
